...
export default function Hero() {
  ...
  // Embed mode skips the standalone marketing landing.
  useEffect(() => {
    if (isEmbedMode) {
      router.replace(withEmbedHref('/dashboard'));
    }
  }, [isEmbedMode, router, withEmbedHref]);

  if (isEmbedMode) {
    return (
      <div
        className="h-full w-full flex items-center justify-center font-mono text-sm text-blue-700"
        data-testid="embed-hero-redirect"
      >
        {t('common.loading')}
      </div>
    );
