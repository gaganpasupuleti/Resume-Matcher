...
export const SwissGrid = ({ children }: { children: React.ReactNode }) => {
  ...
  const grid = (
    <div className="p-[1.5px]">
      <div
        className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 bg-black gap-[1px] border-b border-black"
        data-testid="resume-feature-grid"
      >
        {children}
      </div>
    </div>
  ...
  // Code Quest embed: hide standalone header/footer chrome; full-bleed feature grid.
  if (isEmbedMode) {
    return (
      <div
        className="h-full w-full flex flex-col overflow-hidden bg-[#F0F0E8]"
        data-testid="embed-swiss-grid"
      >
        <div className="shrink-0 px-4 py-3 border-b border-black flex justify-end bg-[#F0F0E8] relative z-30">
          <Link
            href={withEmbedHref('/settings')}
            className="bg-[#F97316] text-black border border-black px-4 py-2 uppercase font-bold tracking-wide shadow-[2px_2px_0px_0px_#000000] hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none transition-all font-mono text-xs"
            data-testid="embed-inner-settings"
          >
            {t('nav.settings')}
          </Link>
        </div>
        <div className="flex-1 overflow-y-auto overflow-x-hidden relative z-10">{grid}</div>
      </div>
    );
  }
  ...
  return (
    ...
    <div
      ...
      data-testid="standalone-swiss-grid"
      style={{
        backgroundImage:
    ...
    >
      ...
      <div className="w-full max-w-[86rem] max-h-full border border-black bg-[#F0F0E8] shadow-[8px_8px_0px_0px_rgba(0,0,0,0.1)] flex flex-col overflow-hidden">
        ...
        <div
          className="border-b border-black p-8 md:p-12 shrink-0 bg-[#F0F0E8] relative z-30"
          data-testid="standalone-dashboard-header"
        >
        ...
        <div
          className="p-4 bg-[#F0F0E8] flex justify-between items-center font-mono text-xs text-blue-700 border-t border-black shrink-0 relative z-30"
          data-testid="standalone-dashboard-footer"
        >
